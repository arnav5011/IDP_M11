...
//Initialise parameters for servo, TOF sensor, and 
int THRESHOLD = 90; // 9cm in millimetres
int DEFAULT_POSITION = 0;
int ACTIVATED_POSITION = 120;
int SERVO_PIN = 12;
...
void moveServo(int targetPosition) {
  ...
  while (currentPosition != targetPosition && isMoving) {
    ...
    // Read magnetic sensors during movement
    int magValue1 = digitalRead(mag_sensor_1); // Read first magnetic sensor
    int magValue2 = digitalRead(mag_sensor_2); // Read second magnetic sensor

    // Determine if the object is magnetic
    if (magValue1 == HIGH || magValue2 == HIGH) { // Assuming HIGH indicates magnetism
      isMagnetic = true;
    } else {
      isMagnetic = false;
    }
